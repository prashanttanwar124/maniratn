...
import { ArrowLeftRight, Coins, RefreshCw, Sparkles } from 'lucide-vue-next';
...
<template>
    ...
    >
        ...
        <div class="grid grid-cols-2 divide-x divide-y divide-surface-200 bg-white text-left text-xs min-[460px]:grid-cols-4 min-[460px]:divide-y-0">
            ...
            <div class="p-2.5">
                <span class="block text-[10px] font-medium text-surface-400">Fine 24K Gold</span>
                <span class="font-mono font-semibold text-surface-800">{{ action.result.fine_gold_weight }}</span>
            </div>
            <div class="p-2.5">
                <span class="block text-[10px] font-medium text-surface-400">Making / GST</span>
                <span class="font-mono font-semibold text-emerald-700">₹0 (None)</span>
            </div>
            <div class="bg-amber-50/50 p-2.5">
                <span class="block text-[10px] font-medium text-amber-800">Net Valuation</span>
                <span class="font-mono text-sm font-bold text-[#9b6f1e]">{{ action.result.total_estimate }}</span>
            </div>
        </div>

        <!-- Footnote -->
        <div class="flex items-center justify-between border-t border-surface-100 bg-surface-50 px-3 py-1.5 text-[10px] text-surface-500">
